...
def scale_by_schedule(step_size_fn: Schedule) -> base.GradientTransformation:
  ...
  """
    ...
    def update_fn(updates, state, inplace=True):
        ...
        if inplace:
            updates = jax.tree_map(lambda g, step_size: g.mul_(step_size),
                                   updates, step_size)
        else:
            updates = jax.tree_map(lambda g, step_size: g.mul(step_size),
                                   updates, step_size)
        return updates, ScaleByScheduleState(
            count=inc_count(updates, state.count))
...
def _update_moment(updates, moments, decay, order, inplace=True):
    ...
    if inplace:
        ...
        def f(g, t):
            return t.mul_(decay).add_(g**order, alpha=1 -
                                      decay) if g is not None else t
    else:

        def f(g, t):
            return t.mul(decay).add(g**order, alpha=1 -
                                    decay) if g is not None else t
  ...
  """
    ...
    def init_fn(params):
        ...
        nu = jax.tree_map(  # Second moment
            lambda t: torch.zeros_like(t, requires_grad=moment_requires_grad),
            params)
        return ScaleByAdamState(count=tuple(0 for _ in range(len(mu))),
                                mu=tuple(mu),
                                nu=tuple(nu))
    ...
    def update_fn(updates, state, inplace=True):
        ...
        if inplace:
            ...
            def f(g, m, v):
                return m.div_(torch.sqrt_(
                    v.add_(eps_root)).add_(eps)) if g is not None else None
        else:

            def f(g, m, v):
                return m.div(torch.sqrt(
                    v.add(eps_root)).add(eps)) if g is not None else None
  ...
  """
    ...
    def init_fn(params):
        ...
        nu = jax.tree_map(  # Second moment
            lambda t: torch.zeros_like(t, requires_grad=moment_requires_grad),
            params)
        return ScaleByAdamState(count=tuple(0 for _ in range(len(params))),
                                mu=mu,
                                nu=nu)
    ...
    def update_fn(updates, state, inplace=True):
        ...
        return tuple(new_updates), ScaleByAdamState(count=count_inc,
                                                    mu=tuple(new_mus),
                                                    nu=tuple(new_nus))
  ...
  """
    ...
    def init_fn(params):
        if decay == 0.:
            ...
            return TraceState(trace=jax.tree_map(
                lambda t: torch.zeros_like(
                    t, requires_grad=moment_requires_grad), params))
...
def scale_by_rms(decay: float = 0.9,
                 eps: float = 1e-8,
