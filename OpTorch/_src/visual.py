# This file is modified from https://github.com/szagoruyko/pytorchviz/blob/master/torchviz/dot.py
# The origin project pytorchviz is released under MIT license
...
def make_dot(var, params=None, show_attrs=False, show_saved=False, max_attr_chars=50):
    ...
    if params is not None:
        from .utils import _ModuleState
        for param in params:
            if isinstance(param, _ModuleState):
                param_map.update({
                    id(k): v for k, v in param.visual_contents.items()})
            else:
                param_map.update({id(k): v for k, v in param.items()})
    ...
    def get_var_name_with_flag(var):
        ...
        if addr in param_map:
            return '%s\n %s' % (param_map[addr][0], size_to_str(param_map[addr][1].size()))
        else:
            return None
