...
def main(args):
    ...
    if args.parallel:
        env = ParallelEnv(NUM_ENVS, lambda_env, selected_keys=["observation", "next_observation", "reward", "action", "done"])
    else:
        env = SerialEnv(NUM_ENVS, lambda_env, selected_keys=["observation", "next_observation", "reward", "action", "done"])
    env.reset()
    # Policy
