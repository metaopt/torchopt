...
from helper.utils import step, sample
...
def main(args):
    ...
    for update in range(args.n_update):
        ...
        for _ in range(n_lookaheads):
            memory1, memory2 = sample(ipd, [agent1.virtual_theta.theta, agent2.theta], [agent1.values, agent2.values], args)
        ...
        for _ in range(n_lookaheads):
            memory1, memory2 = sample(ipd, [agent1.theta, agent2.virtual_theta.theta], [agent1.values, agent2.values], args)
