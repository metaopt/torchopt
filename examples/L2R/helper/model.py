...
class LeNet5(nn.Module):
    def __init__(self, args):
        ...
        self.model = nn.Sequential(nn.Conv2d(1, 16, 5), nn.ReLU(), nn.MaxPool2d(2), nn.Conv2d(16, 32, 5), nn.ReLU(), nn.MaxPool2d(2), nn.Flatten(), nn.Linear(512, 128), nn.ReLU(), nn.Linear(128, 1), nn.Sigmoid())
        self.args = args
        self.meta_weights = torch.zeros(self.args.batch_size, requires_grad=True).to(self.args.device)
    ...
    def inner_loss(self, train_x, train_y):
        ...
        loss = - (train_y * torch.log(result + 1e-10) + (1-train_y) * torch.log(1 - result + 1e-10))
