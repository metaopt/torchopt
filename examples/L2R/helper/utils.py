...
def get_imbalance_dataset(mnist_train,
    ...
    x_train_0 = x_train_0[nval_small_neg:nval_small_neg +
                          ntrain_small_neg]  # 500 4 in training.
    ...
    np.random.shuffle(idx)
    x_test_0 = x_test_0[:int(np.floor(ntest *
                                      ratio_test))]  # 450 4 in testing.
    ...
    x_train_1 = x_train_1[nvalsmall_pos:nvalsmall_pos +
                          ntrainsmall_pos]  # 4500 9 in training.

    idx = np.arange(x_test_1.shape[0])
    np.random.shuffle(idx)
    x_test_1 = x_test_1[idx]
    x_test_1 = x_test_1[:int(np.floor(ntest *
                                      (1 - ratio_test)))]  # 500 9 in testing.
    ...
    x_train_subset, y_train_subset, x_val_subset, y_val_subset, x_test_subset, y_test_subset = torch.tensor(
        x_train_subset), torch.tensor(y_train_subset), torch.tensor(
            x_val_subset), torch.tensor(y_val_subset), torch.tensor(
                x_test_subset), torch.tensor(y_test_subset)

    train_set, val_set, test_set = TensorDataset(
        x_train_subset, y_train_subset), TensorDataset(
            x_val_subset, y_val_subset), TensorDataset(x_test_subset,
                                                       y_test_subset)
